...
const motionPx     = SCROLL_DISTANCE - totalPausePx;

function frameToScrollPx(targetF: number): number {
  let cursorPx = 0, cursorF = 1;
  for (const pause of PAUSE_POINTS) {
    const segmentF  = pause.at - cursorF;
    const segmentPx = (segmentF / INTRO_END_FRAME) * motionPx;
    if (targetF <= pause.at) {
      return cursorPx + (segmentF > 0 ? (targetF - cursorF) / segmentF : 0) * segmentPx;
    }
    cursorPx += segmentPx;
    if (targetF === pause.at) return cursorPx;
    cursorPx += pause.holdPx;
    cursorF   = pause.at;
  }
  const remainF  = INTRO_END_FRAME - cursorF;
  const remainPx = (remainF / INTRO_END_FRAME) * motionPx;
  if (remainF <= 0) return cursorPx;
  return cursorPx + Math.min((targetF - cursorF) / remainF, 1) * remainPx;
}
...
export default function ScrollFrames() {
  ...
  // Timer for FORWARD direction only (tiny debounce to avoid flicker).
  // On REVERSE scroll, buttons hide IMMEDIATELY with no timer.
  const showButtonsTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  /* ================================================================
     SET TARGET FRAME — no button logic here
     ================================================================ */
  const setTargetFrame = useCallback((frame: number) => {
    targetFrameRef.current = frame;
    proxyRef.current.frame = frame;
    ...
    HOTSPOTS.forEach(spot => {
      if (!introCompleteRef.current) {
        ...
        newVis[spot.id] = spot.showDuringSection === SECTIONS[activeSectionRef.current].id;
      } else {
  ...
  /* ================================================================
     START RAF LOOP + GSAP SCROLL once intro frames are ready
     ================================================================ */
  useEffect(() => {
    ...
    const handleScrubUpdate = () => {
      ...
      // ── REVERSE SCROLL: user scrolled back below INTRO_END_FRAME ──
      // Hide buttons IMMEDIATELY — no delay, no timer. Matches video exactly.
      if (frozenRef.current && nf < INTRO_END_FRAME) {
        frozenRef.current        = false;
        introCompleteRef.current = false;
        autoSelectedRef.current  = false;
        activeSectionRef.current = null;

        // Cancel any pending forward-direction show timer
        if (showButtonsTimerRef.current !== null) {
          clearTimeout(showButtonsTimerRef.current);
          showButtonsTimerRef.current = null;
        }

        // Immediate hide — no setTimeout
      ...
      // Only update frame when NOT frozen
      if (!frozenRef.current) {
        setTargetFrame(nf);
      }

      // ── FORWARD: reaching intro end ──
      if (nf >= INTRO_END_FRAME && !introCompleteRef.current) {
        ...
        if (!autoSelectedRef.current) {
          ...
          // 80ms debounce prevents flicker if user skims past frame 200
          if (showButtonsTimerRef.current !== null) clearTimeout(showButtonsTimerRef.current);
          showButtonsTimerRef.current = setTimeout(() => {
            showButtonsTimerRef.current = null;
            // Only commit if we're still in frozen/intro-complete state
            if (frozenRef.current && introCompleteRef.current) {
              setShowButtons(true);
          ...
          activeTweenRef.current = gsap.to(proxy, {
            ...
            onComplete: () => {
              activeTweenRef.current = null;
              snapScrollToFrame(INTRO_END_FRAME);
            },
          });
        }
      }
    ...
    const gsapCtx = gsap.context(() => {
      ...
      PAUSE_POINTS.forEach(p => {
        tl.to(stProxy, { frame: p.at, duration: PLAY, ease: "none", onUpdate: handleScrubUpdate }, t);
        t += PLAY + PAUSE;
      });
      tl.to(stProxy, { frame: INTRO_END_FRAME, duration: PLAY, ease: "none", onUpdate: handleScrubUpdate }, t);
    }, containerRef);
  ...
  }, [readyToShow, updateCanvas, setTargetFrame, startRafLoop]);
  ...
  const handleDotClick = useCallback((dotIndex: number) => {
    ...
    if (!st) return;
    const dotF = [...PAUSE_POINTS.map(p => p.at), INTRO_END_FRAME];
    const px   = frameToScrollPx(dotF[dotIndex]);
    const s = st.start as number, e = st.end as number;
    gsap.to(window, { scrollTo: { y: s + (px / SCROLL_DISTANCE) * (e - s), autoKill: false }, duration: 1.0, ease: "power2.inOut" });
    targetFrameRef.current = dotF[dotIndex];
    proxyRef.current.frame = dotF[dotIndex];
    setActiveSection(null);
    setTargetFrame(dotF[dotIndex]);
  }, [setTargetFrame]);
  ...
  const handleSectionClick = useCallback((sectionIndex: number) => {
    ...
    // Cancel any pending timer and show buttons immediately
    if (showButtonsTimerRef.current !== null) {
      clearTimeout(showButtonsTimerRef.current);
      showButtonsTimerRef.current = null;
    }
    ...
    activeTweenRef.current = gsap.to(proxy, {
      ...
      onComplete: () => {
        ...
        const st = ScrollTrigger.getAll().find(t => t.vars.id === "scroll-frames-trigger");
        if (st) {
          const px = frameToScrollPx(INTRO_END_FRAME);
          const s = st.start as number, e = st.end as number;
          gsap.to(window, { scrollTo: { y: s + (px / SCROLL_DISTANCE) * (e - s), autoKill: false }, duration: 0.5, ease: "power2.inOut" });
        }
      },
    });
  }, [updateCanvas, setTargetFrame]);
  ...
  return (
    <>
      ...
      <div ref={containerRef} style={{ width: "100%", height: "100vh", background: "#000", overflow: "hidden", position: "relative" }}>
        ...
        {/* Progress Bar */}
        <div style={{ position: "absolute", bottom: 0, left: 0, width: "100%", height: 3, background: "rgba(255,255,255,0.1)", zIndex: 10 }}>
          <div ref={progressRef} style={{ height: "100%", transformOrigin: "left center", transform: "scaleX(0)", background: "linear-gradient(90deg, #dc2626, #ef4444)", transition: "none" }} />
        </div>
      </div>
    </>
  );
